 ...
 *
 */
public class SfJdTargetDialog  extends GkBaseDialog {

  /**
   * 
   */
  private static final long serialVersionUID = -4607722811578562848L;
  private SfJdTargetListPanel listPanel;
  private SfJdTargetEditPanel editPanel;
  private SfJdTargetDialog self=this;

  private ForeignEntityDialog forenEntityDialog;

  public SfJdTargetDialog(SfJdTargetListPanel listPanel, List beanList, int editingRow, String tabStatus) {

    super(listPanel.getParentWindow(), Dialog.ModalityType.APPLICATION_MODAL);

    this.listPanel = listPanel;

    editPanel = new SfJdTargetEditPanel(this.self, new ListCursor(beanList, editingRow), tabStatus, listPanel);
...
//    this.setMaxSizeWindow();
  ...
  public SfJdTargetDialog(List beanList, int editingRow, ForeignEntityDialog openDialog) {
    super(openDialog, Dialog.ModalityType.APPLICATION_MODAL);
    this.forenEntityDialog = openDialog;
    editPanel = new SfJdTargetEditPanel(this.self, new ListCursor(beanList, editingRow), this.forenEntityDialog);
    setLayout(new BorderLayout());
    add(editPanel);
    this.setTitle(LangTransMeta.translate("SF_JD_TARGET"));
    this.setSize(UIConstants.DIALOG_3_LEVEL_WIDTH, UIConstants.DIALOG_3_LEVEL_HEIGHT);
    this.moveToScreenCenter();
    this.setVisible(true);
  }
